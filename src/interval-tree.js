...
IntervalTree = (function() {
  ...
  IntervalTree.prototype.add = function(start, end, id) {
    ...
    // insert into tree with all start/end intervals (for range search)
    this.pointTree.insert(new Point(interval.start, id));
    this.pointTree.insert(new Point(interval.end, id));
    this.intervalsById[id] = interval; // for remove and range search
  ...
  // Clears the entire tree, leaving the root behind
  IntervalTree.prototype.clear = function() {
    this.removeSubtree(this.root.left);
    this.removeSubtree(this.root.right);

    for(var k = this.root.starts.length - 1; k >= 0; k--) {
      this.remove(this.root.starts[k].id);
    }

    // console.log('intervals left', this.intervalsById);
    // console.log('nodes left', this.nodesById);
  };

  IntervalTree.prototype.removeSubtree = function(node) {
    if(node === null) return;
    if(node.left !== null) {
    ...
    for(var k = node.starts.length - 1; k >= 0; k--) {
      this.remove(node.starts[k].id);
    }
  }
